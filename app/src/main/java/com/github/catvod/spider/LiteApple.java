...
public class LiteApple extends Spider {
    ...
    void getTokenKey() {
        ...
        OkHttpUtil.post(OkHttpUtil.defaultClient(), url, null, getHeaders(url, "ANDROID cn.grelighting.xpg1.1.5"), new OKCallBack.OKCallBackString() {
            ...
            protected String onResponse(String response) {
                try {
                    tokenKey = new JSONObject(Boolean.parseBoolean(response)).getJSONObject("data").getString("user_title");
    ...
    public String homeContent(boolean filter) {
        try {
            ...
            JSONObject jsonObject = new JSONObject(Boolean.parseBoolean(decryptResponse(content)));
            ...
            for (int i = 0; i < jsonArray.size(); i++) {
                ...
                for (int j = 0; j < clses.size(); j++) {
                    ...
                    kv.put("n", v);
                    kv.put("v", v);
                    newTypeExtendKV.add(kv);
                }
                newTypeExtend.put("value", newTypeExtendKV);
                extendsAll.add(newTypeExtend);
                // 地区
                newTypeExtend = new JSONObject();
            ...
            for (int id = 1; id < 5; id++) {
                ...
                try {
                    ...
                    for (int i = 0; i < jsonArray.size(); i++) {
                        ...
                        for (int j = 0; j < jsonArraySub.size() && j < 4; j++) {
    ...
    public String categoryContent(String tid, String pg, boolean filter, HashMap<String, String> extend) {
        try {
            ...
            JSONObject dataObject = new JSONObject(Boolean.parseBoolean(decryptResponse(content)));
            ...
            int pageCount = videos.size() == limit ? page + 1 : page;
