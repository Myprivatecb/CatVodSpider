...
public class HkTv extends Spider {
    ...
    public String homeContent(boolean filter) throws Exception {
        ...
        for (Element element : doc.select("a.myui-vodlist__thumb")) {
            try {
                ...
                String remark = element.select("span>span").text();
    ...
    public String detailContent(List<String> ids) throws Exception {
        ...
        String info = doc.select("p.data").eachText().get(0);
        String[] parts = info.split(" ");
        // 分配给变量
        String typeName = parts[0].split("：")[1]; // 提取"分类"   parts[0].split("：")[1]
        String area = parts[1].split("：")[1]; // 提取"地区"
        String year = parts[2].split("：")[1]; // 提取"年份"
        String actor = doc.select("p.data").eachText().get(2).split("主演：")[1];
        String director = doc.select("p.data").eachText().get(3).split("导演：")[1];
        String desc = doc.select("p.data").eachText().get(4).split("简介：")[1];
    ...
    public String playerContent(String flag, String id, List<String> vipFlags) throws Exception {
        ...
        if (matcher.find()) {
            ...
            String decodedString = new String(Base64.getDecoder().decode(encryptedData));
            url = decodeURL(decodedString);   //mytv-1b8554a90f2d79fc324bc464b03d12c9
            String jiexiurl = "https://pl.qcheng.cc/hktvpc.php?url=" + url;
            Document doc1 = Jsoup.parse(OkHttp.string(jiexiurl, getHeaders()));
            Element e = doc1.select("body>script").get(0);
            String regex1 = "url: '(.*?)'(?:[,]|$)";
            Pattern pattern1 = Pattern.compile(regex1);
            Matcher matcher1 = pattern1.matcher(e.html());
            if (matcher1.find()) {
